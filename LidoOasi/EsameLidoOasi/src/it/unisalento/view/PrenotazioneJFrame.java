...
import it.unisalento.view.Ascoltatori.Prenotazione.AscoltatoreConfermaPrenotazione;
import it.unisalento.view.Ascoltatori.Prenotazione.AscoltatoreEsciPrenotazione;
import it.unisalento.view.Ascoltatori.Prenotazione.AscoltatorePrenotazioneCabina;
import it.unisalento.view.Ascoltatori.Prenotazione.AscoltatorePrenotazioneConfermaDate;
...
public class PrenotazioneJFrame extends JFrame 
{
	...
	public JButton conferma = new JButton("Conferma Prenotazione");
	private JButton esci = new JButton("Continua");
	...
	private JLabel attenzione = new JLabel("Stampare dopo aver confermato la prenotazione!");
	...
	{
		super("PRENOTAZIONE CABINE LIDO OASI");
	    ...
	    max = PrenotazioneBusiness.getInstance().getMaxIdPrenotazione();  
	    idPrenotazione.setText("           Id Prenotazione : "+(max+1));
		
	
		
		conferma.addActionListener(new AscoltatoreConfermaPrenotazione(this));	
	    confermaDate.addActionListener(new AscoltatorePrenotazioneConfermaDate(this));		
		esci.addActionListener(new AscoltatoreEsciPrenotazione(this)); 
		...
		AscoltatorePrenotazioneCabina listener = new AscoltatorePrenotazioneCabina(this);
		...
		//cabine = SistemaBusiness.getInstance().crea();
		...
		(int i =0; i< 10;i++)
		 ...
		 if( !cab.isPrenotato())
			 bottoni[i].add(new PallinoVerde());
		 else if(cab.isPrenotato())
			 bottoni[i].add(new PallinoGiallo());
		 }
		*/
		
		carrelloAffitto = new CarrelloDellaSpesaAffitto(); 
		pannelloServizi.setCarrello(carrelloAffitto);
		carrelloAffitto.codice.removeActionListener(carrelloAffitto.getAscoltatore()); // rimuove l'ascoltatore
